...
class TestEndpoints:
    ...
    def test_home_page_with_search_term(self, client):
        """Test home page with search term parameter."""
        response = client.get("/?search_term=gatsby")
    ...
    def test_search_books_endpoint(self, client):
        ...
        response = client.get("/search_books?query=gatsby")
    ...
    def test_search_books_empty_query(self, client):
        ...
        response = client.get("/search_books?query=")
    ...
    def test_static_files_mounted(self, client):
        ...
        # This should return 404 for non-existent file but not 500
...
class TestEndpointsNoEngine:
    ...
    def test_home_without_engine(self, client_no_engine):
        """Test home page when engine is None."""
        response = client_no_engine.get("/")
        assert response.status_code == 200
    ...
    def test_search_without_engine(self, client_no_engine):
        ...
        response = client_no_engine.get("/search_books?query=test")
        assert response.status_code == 200
        assert response.json() == []
...
class TestSearchBooksEdgeCases:
    ...
    def test_search_books_with_invalid_image_url(self, client_invalid_image):
        ...
        response = client_invalid_image.get("/search_books?query=test")
    ...
    def mock_engine_few_results(self):
        ...
        engine = MagicMock()
        engine.is_trained = True
        # Only 2 results in books_content, should trigger fallback
        engine.processed_data = {
            "books_content": pd.DataFrame({
                "title": ["Test Book"],
                "author": ["Test Author"],
    ...
    def test_search_books_fallback_to_books(self, client_few_results):
        ...
        response = client_few_results.get("/search_books?query=test")
            ...
            with patch("app.main.RecommendationEngine", return_value=mock_engine):
                with patch("app.main.Config.ensure_directories"):
                    ...
                    async with lifespan(app):
                        # During lifespan, engine should be set up
                        pass
